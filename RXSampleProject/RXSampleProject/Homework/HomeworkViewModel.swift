...
final class HomeworkViewModel {
    enum Action {
        case tableViewItemSelected(Int)
        case searchButtonClicked(String)
        case none
    }
    
    struct State {
        var selectedUsers = [Person]()
    ...
    init() {
        send.bind(with: self) { this, action in
            this.state.accept(this.reducer(action: action))
        }
        .disposed(by: disposeBag)
    }
    
    func reducer(action: Action) -> State {
        var newState = state.value
        
        switch action {
        case let .tableViewItemSelected(row):
            let person = newState.sampleUsers[row]
            newState.selectedUsers.append(person)
            return newState
        case let .searchButtonClicked(text):
            print(text)
            guard !text.isEmpty else {
                newState.sampleUsers = sampleUsers
                return newState
            }
            let filteredUsers = sampleUsers.filter { person in
                return person.name.lowercased().contains(text)
            }
            newState.sampleUsers = filteredUsers
            return newState
        case .none: return newState
        }
    }
}
